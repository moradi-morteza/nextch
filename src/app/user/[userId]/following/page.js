...
function FollowingContent() {
  ...
  const fetchFollowing = async (pageNum, append = false) => {
    try {
      ...
      if (response.data.success) {
        const newFollowing = response.data.data;
    ...
    return (
      <div className="flex-1 flex flex-col bg-white">
        <div className="tg-topbar flex items-center px-3 py-2 sticky top-0 z-40 bg-white border-b border-gray-100" dir="rtl">
          ...
          <h1 className="text-lg font-bold text-black">دنبال شده‌ها</h1>
        </div>
        ...
        <div className="w-full max-w-2xl mx-auto">
          {following.length === 0 ? (
            <div className="text-center py-16 px-4">
              ...
              <h3 className="text-lg font-medium text-gray-700 mb-2">هنوز کسی را دنبال نمی‌کند</h3>
              <p className="text-gray-500">وقتی این کاربر کسی را دنبال کند، اینجا نمایش داده می‌شود</p>
            </div>
          ) : (
            <div>
              {following.map((user) => (
